                ...
                <tbody>
                    {analytics?.recentActivity?.slice(0, 8).map((log, i) => (
                        <tr key={i} style={trStyle}>
                            ...
                            <td style={tdStyle}><span style={{ ...categoryBadge, background: '#E0E7FF', color: '#1E3A8A' }}>{log.category}</span></td>
                        </tr>
                    ))}
                </tbody>
           </table>
...
const SidebarLink = ({ icon, label, active, onClick, danger }) => (
    <div onClick={onClick} style={{
        ...
        marginBottom: '4px', transition: 'all 0.2s', backgroundColor: active ? '#E0E7FF' : 'transparent',
        color: active ? '#1E3A8A' : danger ? '#EF4444' : '#64748B', fontWeight: active ? '800' : '500'
                ...
                <div style={modalOverlay}>
                    <div style={modalContent}>
                        ...
                        <form onSubmit={handleAddStory} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                            ...
                            <textarea style={{ ...formInput, minHeight: '120px' }} placeholder="Story Content Text (optional if Link URL is provided)" value={formData.content} onChange={e => setFormData({...formData, content: e.target.value})} />
                            <input style={formInput} placeholder="Story Link URL (External website or PDF link)" value={formData.pdfUrl || ''} onChange={e => setFormData({...formData, pdfUrl: e.target.value})} />
                            <p style={{ fontSize: '12px', color: '#64748B', margin: '-8px 0 0 0' }}>💡 Tip: Add a working link (e.g. story website or PDF) so elders can open and read it directly.</p>
...
const SettingsView = ({ showToast }) => {
    return (
        <div style={{ ...fadeAnimation, maxWidth: '1000px' }}>
            <h3 style={{ fontSize: '28px', fontWeight: '900', color: '#0F172A', marginBottom: '8px', fontFamily: "'Outfit', sans-serif" }}>System Settings</h3>
            <p style={{ color: '#64748B', marginBottom: '32px', fontSize: '16px' }}>Configure your administrative profile and platform preferences.</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '24px' }}>
                ...
                <div style={adminCardStyle}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
                        <div style={{ background: '#E0E7FF', padding: '10px', borderRadius: '12px', color: '#1E3A8A' }}>
                            <UserCircle size={24} />
                        </div>
                        <h4 style={cardTitleStyle}>Admin Profile</h4>
                    </div>
                    
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                            <label style={{ fontSize: '13px', fontWeight: '800', color: '#94A3B8' }}>DISPLAY NAME</label>
                            <input style={formInput} placeholder="Display Name" defaultValue="System Admin" />
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                            <label style={{ fontSize: '13px', fontWeight: '800', color: '#94A3B8' }}>EMAIL ADDRESS</label>
                            <input style={formInput} placeholder="Email" defaultValue="admin@elderease.com" disabled />
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                            <label style={{ fontSize: '13px', fontWeight: '800', color: '#94A3B8' }}>NEW PASSWORD</label>
                            <input style={formInput} placeholder="Leave empty to keep current" type="password" />
                        </div>
                        <button style={{ ...btnActionStyle, marginTop: '10px', justifyContent: 'center' }} onClick={() => showToast("Profile settings saved")}>
                            Update Profile
                        </button>
                    </div>
                </div>
                ...
                <div style={adminCardStyle}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
                        ...
                        <h4 style={cardTitleStyle}>Security & Platform</h4>
                    </div>

                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        <SettingToggle label="Two-Factor Authentication" description="Add an extra layer of security to your account." active={true} />
                        <SettingToggle label="System Notifications" description="Receive alerts for high-severity SOS triggers." active={true} />
                        <SettingToggle label="Auto-Backup Database" description="Securely backup system data every 24 hours." active={false} />
                        <SettingToggle label="Maintenance Mode" description="Disable public access for system updates." active={false} />
                    </div>
                    
                    <div style={{ marginTop: '32px', padding: '16px', borderRadius: '12px', background: '#F8FAFC', border: '1px solid #E2E8F0' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div>
                                <div style={{ fontSize: '14px', fontWeight: '800', color: '#0F172A' }}>Storage Usage</div>
                                <div style={{ fontSize: '12px', color: '#64748B' }}>1.2GB of 10GB used</div>
                            </div>
                            <div style={{ fontWeight: '800', color: '#1E3A8A' }}>12%</div>
                        </div>
                        <div style={{ height: '8px', background: '#E2E8F0', borderRadius: '4px', marginTop: '10px', overflow: 'hidden' }}>
                            <div style={{ width: '12%', height: '100%', background: '#1E3A8A', borderRadius: '4px' }} />
                        </div>
                    </div>
                </div>
            </div>
...
const logoIcon = { width: '40px', height: '40px', background: '#1E3A8A', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center' };
...
const avatarStyle = { width: '36px', height: '36px', background: '#E0E7FF', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '800', color: '#1E3A8A', fontSize: '13px' };
...
const btnActionStyle = { padding: '10px 20px', background: '#1E3A8A', color: 'white', border: 'none', borderRadius: '12px', fontWeight: '800', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px' };
const fullBtnStyle = { width: '100%', padding: '12px', background: '#F8FAFC', color: '#1E3A8A', border: '1px solid #E0E7FF', borderRadius: '12px', fontWeight: '800', cursor: 'pointer', fontSize: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' };
...
const textBtnStyle = { border: 'none', background: 'transparent', color: '#1E3A8A', fontWeight: '800', cursor: 'pointer' };
