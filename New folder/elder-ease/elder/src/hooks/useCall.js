...
export const useCall = (userId) => {
    ...
    // Ref mirrors the state so socket handlers inside useEffect always read the latest value
    ...
    const startWebRTC = async (targetUserId, isInitiator) => {
        try {
            ...
            setCallError('Could not access microphone. Please allow microphone access.');
            setCallState('idle');
        }
    };
    ...
    useEffect(() => {
        ...
        // Use remoteUserIdRef here — state would be stale (captured at effect setup time)
        socket.on('call-accepted', () => {
            startWebRTC(remoteUserIdRef.current, true);
        });
        ...
        socket.on('webrtc-offer', async ({ from, offer }) => {
            // Elder also handles incoming offers (in case family initiates later)
    ...
    const initiateCall = useCallback((targetUserId, callerName) => {
        setCallError('');
        updateRemoteUserId(targetUserId);
    ...
    const acceptCall = useCallback(() => {
        ...
        setIncomingCall(null);
        startWebRTC(incomingCall.from, false);
    }, [incomingCall]);
